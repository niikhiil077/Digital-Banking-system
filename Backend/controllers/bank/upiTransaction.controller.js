...
export const upiTransactionController = asyncHandler(async (req, res, next) => {
    ...
    if (!pin || !recieverUPIid || !amount) {
        throw new Error('Please provide recivers UPI id and your UPI pin with amount ');
    }
    ...
    if (amount > bank.balance || amount <= 0) {
        throw new Error('Insufficient Balance || amount cannot be negative or zero');
    }
    ...
    try {
        ...
        const updatedSenderAccount = await Bank.findOneAndUpdate({ accountHolderId: req.card.userId }, {
            $inc: { balance: -amount }
        }, {
        ...
        const updatedRecieverAcccount = await Bank.findOneAndUpdate({ accountHolderId: reciever.cardHolderUserId }, {
            $inc: { balance: amount }
        }, {
            returnDocument: "after"
        }).session(session)
