      ...
      try {
        ...
        if (totalTransaction >= 200000) {
          throw new errorResponse(
            ...
            "You have reached your maximum transfer limit over 200000...try again tomorrow",
          );
        }
        ...
        if (totalTransaction + data.amount > 200000) {
          throw new errorResponse(
            400,
            "unsuffiecient Limit",
            `You remaining limit is just ${200000 - totalTransaction}`,
          );
        }
        ...
        if (data.amount > 100000) {
          throw new errorResponse(
            400,
            "Unauthorized",
            "You cannot send more than 1 lakhs at one time",
          );
        }
        ...
        if (!updatedSenderAcc) {
          throw new errorResponse(
            500,
            "Transaction failed",
            "Transaction failure while debiting amount from senders bank",
          );
        }
        ...
        if (!updatedRecieverAcc) {
          throw new errorResponse(
            500,
            "Transacation failed",
            "Transaction failure while crediting amount to recievers bank",
          );
        }
        ...
        return res.status(200).json({
          ...
          amount: data.amount,
          beneficiaryMessage: req.beneficiaryMessage || null,
        });
      } catch (error) {
      ...
      try {
        ...
        if (data.amount > 200000) {
          throw new errorResponse(
            400,
            "Unauthorized",
            "You cannot send more than 2 lakhs at one time",
          );
        }
      ...
      try {
        ...
        if (data.amount < 200000) {
          throw new errorResponse(
            400,
            "Unauthorized",
            "You cannot send less than 2 lakhs in this mode of transaction {RTGS} ",
          );
        }
