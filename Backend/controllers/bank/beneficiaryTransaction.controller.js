...
export const beneficiaryTransactionController = asyncHandler(
  async (req, res, next) => {
    ...
    if (!list.length) {
      throw new Error(
        "No beneficiary found with this bank id...{Add beneficiary via once doing netbanking payment} ",
      );
    }
    ...
    if (sendersBank.balance < amount || amount <= 0) {
      throw new Error(
        "Invalid / Insufficient amount ... {Amount can nevr be zero or negative or bank balance is not sufficient} ",
      );
    }
